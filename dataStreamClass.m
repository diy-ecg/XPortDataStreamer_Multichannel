...
classdef dataStreamClass < handle
    ...
    properties
        ...
        #ylim      = [-100 100];    #alternativ lim = [0 100];
        filter    = 1;
    ...
    methods (Access=public)
        ...
        function addSamples(self,samples,timestamps)
          ...
          for k = 1:length(samples)
            ...
            self.dynamic_threshold = 0.5 * max(self.max_buffer); % Adaptives Maximum über die letzten 2 Sekunden
            % R-Zacken-Erkennung
            if sample > self.dynamic_threshold && (timestamp - self.last_r_peak_time) > 250 % Mindestens 250ms Abstand >> Puls 240
